...
func value(data DataBase, form string, column int, row int) string {
	...
	if cell_data == nil {
		...
		} else if is_interface_a_bool(cell) {
			if interface_to_bool(cell) {
				value = "true"
			} else {
				value = "false"
			}
...
// find widest cells
func FindWidestWidths(form string, data DataBase) []int {
	if 1 > len(form) {
		form = "main"
	}
	...
	for r := 0; r < row_count; r++ {
		for c, column_name := range data.Forms[form] {
			if r == 0 {
				widths[c] = len(column_name)
			}
			cell_value_length := len(value(data, form, c, r))
			widths[c] = int(math.Max(float64(cell_value_length), float64(widths[c])))
		}
	}
	return widths
}
...
func Sub(args []string, data DataBase) {
	...
	widths := FindWidestWidths(form, data)
...
func ProcessLine(raw string, app_data *AppData) DataBase {
	...
	case "q", "quit", "exit":
		if app_data.Verbose {
			fmt.Printf("getting out of here\n")
		}
	...
	case "rpn":
		if 1 < len(args) && 0 < len(args[0]) {
			row, _ := strconv.Atoi(args[0])
			formula := strings.Join(args[1:], " ")
			fmt.Printf("%d: %s = %s\n", row, formula,
				formula_for_data(formula, row, app_data.Data))
			fmt.Println("done")
		} else {
	...
	case "FF", "Form":
		...
		FormFiller(form, action, &app_data.Data) //TODO: not done
	case "ff", "form":
		form := ""
		action := "create"
		if 0 < len(args) {
