...
func num_or_str(foo func(data []interface{}) (float64, string), data []interface{}) string {
	result := ""
	value, text := foo(data)
	if text == "" {
		result = fmt.Sprintf(app_data.Format.template_float, value)
	} else {
		result = fmt.Sprintf(app_data.Format.template_string, text)
	}
	return result
}
...
func SumMax(data []interface{}) (float64, string) {
	max := math.SmallestNonzeroFloat64
	text := ""
	for _, value := range data {
		if is_interface_a_number(value) {
			max = math.Max(max, interface_to_float(value))
		} else if is_interface_a_string(value) {
			parsed := interface_to_string(value)
			if text == "" || parsed > text {
				text = parsed
			}
		}
...
func SumMin(data []interface{}) (float64, string) {
	min := math.MaxFloat64
	text := ""
	for _, value := range data {
		if is_interface_a_number(value) {
